...
#
# La LSTM procesa las 24h en orden temporal (many-to-one).
# Mayor capacidad que T=1h/2h/4h por la complejidad de salida.
# =============================================================
...
print(f"LSTM T=24h | Train: {len(X_train)} dias  Test: {len(X_test)} dias  Shape: {X_train.shape}")
...
for i, dia_id in enumerate(dias_test):
    for h in range(T):
        filas.append({
            'DIA': dia_id, 'HORA': h, 'HORIZONTE': 'T=24h', 'MODELO': 'LSTM',
...
df_res.to_csv(os.path.join(DIR, 'LSTM_T24h_resultados.csv'), index=False)
...
}]).to_csv(os.path.join(DIR, 'LSTM_T24h_metricas.csv'), index=False)

DIA_VIS = dias_test[0]
d0 = df_res[df_res['DIA']==DIA_VIS].sort_values('HORA')
fig, axes = plt.subplots(1, 3, figsize=(15,4))
fig.suptitle('LSTM - T=24h', fontsize=13)
...
plt.tight_layout()
plt.savefig(os.path.join(DIR, 'LSTM_T24h_graficas.png'), dpi=150)
plt.close()
print(f"\n  Archivos guardados en: {DIR}")
print("  *** LSTM T=24h COMPLETADO ***\n")