...
#
# Entrada (4 features):
#   precio(t), pv(t), demanda(t), SOC(t-1)
# Salida (2 outputs):
...
print(f"MLP T=1h | Train: {len(X_train):,}  Test: {len(X_test):,}  Entrada: {X_train.shape[1]} features")
...
# =============================================================
# ARQUITECTURA MLP  4 → 64 → 128 → 64 → 2
# =============================================================
...
}]).to_csv(os.path.join(DIR, 'MLP_T1h_metricas.csv'), index=False)
...
fig.suptitle('MLP — T=1h', fontsize=13)
...
plt.tight_layout()
plt.savefig(os.path.join(DIR, 'MLP_T1h_graficas.png'), dpi=150)
plt.close()
print(f"\n  Archivos guardados en: {DIR}")
print("  *** MLP T=1h COMPLETADO ***\n")