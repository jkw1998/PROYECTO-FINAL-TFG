...
#
# Entrada (16 features = 4 pasos x 4 variables):
#   [feat(t-3), feat(t-2), feat(t-1), feat(t)]
#   feat(tau) = [precio(tau), pv(tau), demanda(tau), SOC(tau-1)]
# Salida (2 outputs):
#   p_bat(t), SOC(t)  — solo la hora actual
#
# Padding para t < 3: se replican los valores disponibles
# =============================================================
...
for _, row in df.iterrows():
    ...
    for t in range(T):
        x = []
        for delta in range(W-1, -1, -1):   # t-3, t-2, t-1, t
            x.extend(get_feat(row, t - delta, soc0))
        filas_X.append(x)
...
print(f"MLP T=4h | Train: {len(X_train):,}  Test: {len(X_test):,}  Entrada: {X_train.shape[1]} features")
...
# =============================================================
# ARQUITECTURA MLP  16 → 64 → 128 → 64 → 2
# =============================================================
...
}]).to_csv(os.path.join(DIR, 'MLP_T4h_metricas.csv'), index=False)
...
fig.suptitle('MLP — T=4h', fontsize=13)
...
plt.tight_layout()
plt.savefig(os.path.join(DIR, 'MLP_T4h_graficas.png'), dpi=150)
plt.close()
print(f"\n  Archivos guardados en: {DIR}")
print("  *** MLP T=4h COMPLETADO ***\n")