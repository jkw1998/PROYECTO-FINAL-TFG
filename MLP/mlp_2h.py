...
#
# Entrada (8 features = 2 pasos x 4 variables):
#   [precio(t-1), pv(t-1), demanda(t-1), SOC(t-2),
#    precio(t),   pv(t),   demanda(t),   SOC(t-1)]
# Salida (2 outputs):
#   p_bat(t), SOC(t)  — solo la hora actual
#
# Padding para t=0: se replican los valores de t=0
# =============================================================
...
print(f"MLP T=2h | Train: {len(X_train):,}  Test: {len(X_test):,}  Entrada: {X_train.shape[1]} features")
...
# =============================================================
# ARQUITECTURA MLP  8 → 64 → 128 → 64 → 2
# =============================================================
...
}]).to_csv(os.path.join(DIR, 'MLP_T2h_metricas.csv'), index=False)
...
fig.suptitle('MLP — T=2h', fontsize=13)
...
plt.tight_layout()
plt.savefig(os.path.join(DIR, 'MLP_T2h_graficas.png'), dpi=150)
plt.close()
print(f"\n  Archivos guardados en: {DIR}")
print("  *** MLP T=2h COMPLETADO ***\n")