# =============================================================
# TFG - MODELO IA: MLP — HORIZONTE T=2h
# =============================================================
...
# Dataset de entrada:
# -------------------
# Dataset generado con horizonte T=2h
# (ventana deslizante de 2h, paso 1h, SOC encadenado).
#
...
df = pd.read_csv(os.path.join(DIR, "dataset_vpp_ia_T2h.csv"))
...
csv_path = os.path.join(DIR, "mlp_resultados_T2h.csv")
...
fig.suptitle(f"MLP — T=2h", fontsize=13)
...
axes[1].set_title(f"Accion bateria (dia 800) — T=2h")
...
axes[2].plot(dia0["HORA"], dia0["SOC_IA"],
             label="MLP", color='darkorange', linestyle='--')
axes[2].set_title(f"SOC (dia 800) — T=2h")
axes[2].set_xlabel("Hora")
axes[2].legend()
axes[2].grid(True, alpha=0.4)

plt.tight_layout()
png_path = os.path.join(DIR, "mlp_graficas_T2h.png")
