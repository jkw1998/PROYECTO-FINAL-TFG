# =============================================================
# TFG - MODELO IA: MLP (Multilayer Perceptron)
# =============================================================
...
# recibe las mismas entradas y predice directamente las
# mismas salidas, sin ninguna optimizacion en tiempo real.
#
...
# --- Normalizacion de entradas (solo fit en train) ---
...
class MLP(nn.Module):
    def __init__(self):
        ...
        self.red = nn.Sequential(
            ...
            nn.Linear(42, 48)               # 24 p_bat + 24 soc
        )
    def forward(self, x):
        return self.red(x)
...
df_res.to_csv("mlp_resultados.csv", index=False)
...
fig.suptitle("MLP - Evaluacion", fontsize=13)
...
axes[1].plot(dia0["HORA"], dia0["ACCION_IA"],   label="MLP",  color='darkorange', linestyle='--')
...
axes[2].plot(dia0["HORA"], dia0["SOC_IA"],   label="MLP",  color='darkorange', linestyle='--')
axes[2].set_title("SOC (dia 800)"); axes[2].set_xlabel("Hora"); axes[2].legend(); axes[2].grid(True, alpha=0.4)

plt.tight_layout()
plt.savefig("mlp_graficas.png", dpi=150)
plt.show()