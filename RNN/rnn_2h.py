...
#
# Entrada: secuencia de longitud 2 x 4 features
#   (batch, seq_len=2, input_size=4)
#   paso 0: [precio(t-1), pv(t-1), demanda(t-1), SOC(t-2)]
#   paso 1: [precio(t),   pv(t),   demanda(t),   SOC(t-1)]
# Salida (2 outputs):
#   p_bat(t), SOC(t) — solo la hora actual
#
# La RNN procesa los 2 pasos en orden temporal y usa
...
# =============================================================
# CARGA Y CONSTRUCCIÓN DEL DATASET — T=2h
# =============================================================
...
for _, row in df.iterrows():
    ...
    for t in range(T):
        ...
        # Paso anterior (t-1), padding si t=0
        if t == 0:
        ...
        # Secuencia: [paso t-1, paso t]  shape (2, 4)
...
X_all   = np.array(filas_X, dtype=np.float32)   # (N, 2, 4)
...
print(f"RNN T=2h | Train: {len(X_train):,}  Test: {len(X_test):,}  "
      f"Shape entrada: {X_train.shape}  (batch, seq=2, feat=4)")

# =============================================================
# NORMALIZACIÓN (sobre dimension features)
# =============================================================
...
}]).to_csv(os.path.join(DIR, 'RNN_T2h_metricas.csv'), index=False)
...
fig.suptitle('RNN — T=2h', fontsize=13)
...
axes[1].set_title(f'Accion bateria (dia {DIA_VIS}) — T=2h')
...
axes[2].set_title(f'SOC (dia {DIA_VIS}) — T=2h')
axes[2].set_xlabel('Hora'); axes[2].set_ylabel('SOC (p.u.)')
axes[2].legend(); axes[2].grid(True, alpha=0.4)

plt.tight_layout()
plt.savefig(os.path.join(DIR, 'RNN_T2h_graficas.png'), dpi=150)
plt.close()
print(f"\n  Archivos guardados en: {DIR}")
print("  *** RNN T=2h COMPLETADO ***\n")