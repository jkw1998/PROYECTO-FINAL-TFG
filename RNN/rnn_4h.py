...
#
# Entrada: secuencia de longitud 4 x 4 features
#   (batch, seq_len=4, input_size=4)
...
#
# La RNN procesa los 4 pasos en orden temporal y usa
# el estado oculto del ultimo paso para la prediccion.
...
# =============================================================
# CARGA Y CONSTRUCCIÓN DEL DATASET — T=4h
# =============================================================
...
for _, row in df.iterrows():
    ...
    for t in range(T):
        # Secuencia: [feat(t-3), feat(t-2), feat(t-1), feat(t)]
        seq = [get_feat(row, t - delta, soc0)
               for delta in range(W-1, -1, -1)]   # shape (4, 4)
        filas_X.append(seq)
        filas_y.append([float(row[f'p_bat_h{t}']), float(row[f'soc_h{t}'])])
        meta.append({'dia_id': int(row['dia_id']),
                     'hora': t, 'split': row['split']})

X_all   = np.array(filas_X, dtype=np.float32)   # (N, 4, 4)
...
print(f"RNN T=4h | Train: {len(X_train):,}  Test: {len(X_test):,}  "
      f"Shape entrada: {X_train.shape}  (batch, seq=4, feat=4)")
...
# =============================================================
# ARQUITECTURA RNN
# RNN(input=4, hidden=64, layers=2) + FC(64->32->2)
# =============================================================
...
}]).to_csv(os.path.join(DIR, 'RNN_T4h_metricas.csv'), index=False)
...
fig.suptitle('RNN — T=4h', fontsize=13)
...
axes[1].set_title(f'Accion bateria (dia {DIA_VIS}) — T=4h')
...
axes[2].set_title(f'SOC (dia {DIA_VIS}) — T=4h')
axes[2].set_xlabel('Hora'); axes[2].set_ylabel('SOC (p.u.)')
axes[2].legend(); axes[2].grid(True, alpha=0.4)

plt.tight_layout()
plt.savefig(os.path.join(DIR, 'RNN_T4h_graficas.png'), dpi=150)
plt.close()
print(f"\n  Archivos guardados en: {DIR}")
print("  *** RNN T=4h COMPLETADO ***\n")