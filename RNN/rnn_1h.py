# =============================================================
# TFG - RNN | HORIZONTE T=1h
# =============================================================
# Autor: James Kagunda Wangari
# Grado en Ingenieria Electrica - Universidad de Malaga
#
# Arquitectura: RNN simple (recurrente)
# Horizonte:    T=1h — la IA ve UNA sola hora cada vez
#
# Entrada: secuencia de longitud 1 x 4 features
#   (batch, seq_len=1, input_size=4)
#   features: precio(t), pv(t), demanda(t), SOC(t-1)
# Salida (2 outputs):
#   p_bat(t), SOC(t) — solo la hora actual
#
# Para T=1h la secuencia tiene longitud 1, por lo que
# la RNN actua como un mapper no lineal, igual que el MLP.
# La diferencia real en RNN aparece en T=2h y T=4h.
#
# Dataset: 1000 dias x 24h = 24.000 muestras
# Referencia: solucion optima del optimizador MILP 24h
# =============================================================
...
# =============================================================
# CARGA Y CONSTRUCCIÓN DEL DATASET — T=1h
# =============================================================
...
for _, row in df.iterrows():
    ...
    for t in range(T):
        soc_prev = soc0 if t == 0 else float(row[f'soc_h{t-1}'])
        filas_X.append([[
            float(row[f'precio_h{t}']),
            float(row[f'pv_h{t}']),
            float(row[f'demanda_h{t}']),
            soc_prev
        ]])   # shape (1, 4) — secuencia de longitud 1
        filas_y.append([float(row[f'p_bat_h{t}']), float(row[f'soc_h{t}'])])
        meta.append({'dia_id': int(row['dia_id']),
                     'hora': t, 'split': row['split']})

# X: (N, seq_len=1, features=4)
...
print(f"RNN T=1h | Train: {len(X_train):,}  Test: {len(X_test):,}  "
      f"Shape entrada: {X_train.shape}  (batch, seq=1, feat=4)")

# =============================================================
# NORMALIZACIÓN
# Normalizamos sobre la vista 2D y luego reponemos la dim seq
# =============================================================
...
# =============================================================
# ARQUITECTURA RNN
# RNN(input=4, hidden=64, layers=2) + FC(64->2)
# =============================================================
...
class RNNModel(nn.Module):
    ...
    def forward(self, x):
        ...
        out = out[:, -1, :]         # ultimo paso de la secuencia
        return self.fc(out)
...
}]).to_csv(os.path.join(DIR, 'RNN_T1h_metricas.csv'), index=False)
...
fig.suptitle('RNN — T=1h', fontsize=13)
...
axes[1].plot(d0['HORA'], d0['ACCION_IA'],   label='RNN', color='darkorange', ls='--')
axes[1].set_title(f'Accion bateria (dia {DIA_VIS}) — T=1h')
...
axes[2].plot(d0['HORA'], d0['SOC_IA'],   label='RNN', color='darkorange', ls='--')
axes[2].set_title(f'SOC (dia {DIA_VIS}) — T=1h')
axes[2].set_xlabel('Hora'); axes[2].set_ylabel('SOC (p.u.)')
axes[2].legend(); axes[2].grid(True, alpha=0.4)

plt.tight_layout()
plt.savefig(os.path.join(DIR, 'RNN_T1h_graficas.png'), dpi=150)
plt.close()
print(f"\n  Archivos guardados en: {DIR}")
print("  *** RNN T=1h COMPLETADO ***\n")