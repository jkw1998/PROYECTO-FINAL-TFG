# =============================================================
# TFG - MODELO IA: MLP — HORIZONTE T=4h
# =============================================================
# Autor: James Kagunda Wangari
# Grado en Ingenieria Electrica - Universidad de Malaga
#
# Red neuronal feedforward. Trata las 24h como vector
# plano sin estructura temporal explícita.
#
# Dataset de entrada:
# -------------------
# Dataset generado con horizonte T=4h
# (ventana deslizante de 4h, paso 3h, SOC encadenado).
#
# Entradas (73): soc_inicial + precio*24 + pv*24 + demanda*24
# Salidas  (48): p_bat*24 + soc*24
#
# Arquitectura: MLP: 73 -> 50 -> 80 -> 42 -> 48
...
df = pd.read_csv(os.path.join(DIR, "dataset_vpp_ia_T4h.csv"))
...
for epoch in range(EPOCHS):
    model.train()
    optimizer.zero_grad()
    loss = criterion(model(X_train_t), torch.FloatTensor(y_train))
    loss.backward()
    optimizer.step()
...
csv_path = os.path.join(DIR, "mlp_resultados_T4h.csv")
...
fig.suptitle(f"MLP — T=4h", fontsize=13)
...
axes[1].plot(dia0["HORA"], dia0["ACCION_IA"],
             label="MLP", color='darkorange', linestyle='--')
axes[1].set_title(f"Accion bateria (dia 800) — T=4h")
...
axes[2].plot(dia0["HORA"], dia0["SOC_IA"],
             label="MLP", color='darkorange', linestyle='--')
axes[2].set_title(f"SOC (dia 800) — T=4h")
axes[2].set_xlabel("Hora")
axes[2].legend()
axes[2].grid(True, alpha=0.4)

plt.tight_layout()
png_path = os.path.join(DIR, "mlp_graficas_T4h.png")
