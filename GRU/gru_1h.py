...
#
# Entrada: secuencia (batch, seq_len=1, input_size=4)
#   features: precio(t), pv(t), demanda(t), SOC(t-1)
# Salida (2 outputs): p_bat(t), SOC(t)
#
# GRU vs LSTM: menos parametros (sin celda de memoria separada),
# similar capacidad en secuencias cortas, mas rapido de entrenar.
...
for _, row in df.iterrows():
    ...
    for t in range(T):
        ...
        filas_X.append([[float(row[f'precio_h{t}']), float(row[f'pv_h{t}']),
                         float(row[f'demanda_h{t}']), soc_prev]])
...
print(f"GRU T=1h | Train: {len(X_train):,}  Test: {len(X_test):,}  Shape: {X_train.shape}")
...
}]).to_csv(os.path.join(DIR, 'GRU_T1h_metricas.csv'), index=False)

DIA_VIS = df_res['DIA'].unique()[0]
d0 = df_res[df_res['DIA']==DIA_VIS].sort_values('HORA')
fig, axes = plt.subplots(1, 3, figsize=(15,4))
fig.suptitle('GRU - T=1h', fontsize=13)
...
axes[1].set_title(f'Accion bateria (dia {DIA_VIS}) - T=1h'); axes[1].set_xlabel('Hora'); axes[1].set_ylabel('P_bat (MW)'); axes[1].legend(); axes[1].grid(True, alpha=0.4)
...
axes[2].set_title(f'SOC (dia {DIA_VIS}) - T=1h'); axes[2].set_xlabel('Hora'); axes[2].set_ylabel('SOC (p.u.)'); axes[2].legend(); axes[2].grid(True, alpha=0.4)
plt.tight_layout()
plt.savefig(os.path.join(DIR, 'GRU_T1h_graficas.png'), dpi=150)
plt.close()
print(f"\n  Archivos guardados en: {DIR}")
print("  *** GRU T=1h COMPLETADO ***\n")