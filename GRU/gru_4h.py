...
print(f"GRU T=4h | Train: {len(X_train):,}  Test: {len(X_test):,}  Shape: {X_train.shape}")
...
}]).to_csv(os.path.join(DIR, 'GRU_T4h_metricas.csv'), index=False)

DIA_VIS = df_res['DIA'].unique()[0]
d0 = df_res[df_res['DIA']==DIA_VIS].sort_values('HORA')
fig, axes = plt.subplots(1, 3, figsize=(15,4))
fig.suptitle('GRU - T=4h', fontsize=13)
...
plt.tight_layout()
plt.savefig(os.path.join(DIR, 'GRU_T4h_graficas.png'), dpi=150)
plt.close()
print(f"\n  Archivos guardados en: {DIR}")
print("  *** GRU T=4h COMPLETADO ***\n")